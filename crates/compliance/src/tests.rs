...
fn package_checks_are_case_insensitive() {
    ...
    assert!(result
        .checks
        .iter()
        .any(|c| { c.check_type == "package_installed" && c.status == "compliant" }));
    assert!(result
        .checks
        .iter()
        .any(|c| { c.check_type == "package_not_installed" && c.status == "non_compliant" }));
}
...
fn missing_probe_values_fail_strict_checks() {
    ...
    assert!(result
        .checks
        .iter()
        .any(|c| c.check_type == "ssh_config" && c.status == "non_compliant"));
}
...
fn required_services_check_fails_when_service_not_running() {
    ...
    assert!(result
        .checks
        .iter()
        .any(|c| c.check_type == "service" && c.status == "non_compliant"));
}
...
fn antivirus_check_uses_probe_flag() {
    ...
    assert!(result
        .checks
        .iter()
        .any(|c| c.check_type == "antivirus" && c.status == "non_compliant"));
}
...
fn os_version_gte_check_compares_numeric_prefix() {
    ...
    assert!(result
        .checks
        .iter()
        .any(|c| c.check_type == "os_version" && c.status == "compliant"));
}
...
fn firewall_required_check_maps_to_expected_fail_and_remediation() {
    ...
    assert!(result
        .checks
        .iter()
        .any(|c| c.check_type == "firewall" && c.status == "non_compliant"));
