...
14. [eGuard API Server (Perl to Go Migration)](#14-eguard-api-server-perl--go-migration)
...
## 14. eGuard API Server (Perl to Go Migration)

The eGuard management API has been rewritten from Perl (Mojolicious prefork) to
Go (net/http + httprouter). This section documents the architecture, deployment,
authentication flow, and operational procedures for the new `eg-api-server`.

### 14.1 Motivation

The legacy Perl API (`egperl-api`) used a prefork concurrency model that was
resource-intensive and difficult to maintain at scale:

| Metric | Perl (egperl-api) | Go (eg-api-server) | Improvement |
|--------|-------------------|--------------------|-------------|
| RAM (idle) | ~3.4 GB (11 prefork workers) | ~20 MB (single binary) | 99.4% reduction |
| Startup time | 5--10 seconds | < 1 second | 5--10x faster |
| CPU (idle) | 10--30% | ~0% | Negligible idle cost |
| Concurrency model | Prefork (fixed worker pool) | Goroutines (per-request) | Scales to thousands of concurrent requests |
| Type system | Dynamic (Perl) | Static (Go) | Compile-time safety, easier refactoring |
| Binary deployment | Perl + CPAN modules + system deps | Single static binary | No runtime dependencies |

### 14.2 Architecture (Before vs After)

**Before (Perl):**

```
Browser --> Caddy (api-aaa auth plugin)
                |
                v
        egperl-api (port 22224, Mojolicious prefork, 11 workers)
                |
                v
             MySQL
```
...
Browser --> Caddy (api-aaa auth plugin)
                |
                v
        eg-api-server (port 22230, single binary, goroutine-per-request)
                |
                v
        MySQL (GORM, MaxIdle=5, MaxOpen=50, ConnMaxLifetime=5m)
```

Both architectures sit behind Caddy's `api-aaa` authentication plugin, which
intercepts all `/api/v1/*` requests and validates credentials before forwarding
to the backend. The Go server is a drop-in replacement -- no client-side changes
are required.

### 14.3 Package Structure

```
go/cmd/eg-api-server/          Binary entrypoint (sd_notify, graceful shutdown)
go/apiserver/
    server.go                  HTTP server setup, middleware chain, lifecycle
    router.go                  Main route registration (89 routes)
    routes_specialized.go      Specialized routes (60 routes)
    routes_config.go           INI config CRUD controllers (45 controllers)
    middleware.go              Recovery + CORS middleware
    handler/                   40 handler files (one per resource domain)
    store/                     31 GORM model files (one per DB table)
    crud/                      Generic CRUD handler (search, list, get, create, update, delete)
    configcrud/                INI config CRUD (search + handler for .conf files)
    httputil/                  Shared HTTP utilities (JSON read/write, pagination)
    search/                    Search/filter query builder
```

**Router**: `github.com/wwicak/httprouter v1.0.0` (forked from julienschmidt/httprouter
with proper Go module path).

### 14.4 Route Inventory (194 Total)

| File | Route Count | Coverage |
|------|-------------|----------|
| `router.go` | 89 | Agents, events, compliance, policy, commands, responses, incidents, threat-intel, users, nodes, services, reports, authentication |
| `routes_specialized.go` | 60 | DHCP, fingerbank, switches, connection profiles, provisioning, PKI, firewalls, floating devices, security events |
| `routes_config.go` | 45 | INI-file-backed config controllers (admin roles, security events, roles, realms, sources, scans, maintenance, etc.) |

Additionally, a `/healthz` endpoint is registered for liveness probes.

### 14.5 Authentication Flow

The Go server does not perform authentication on every request. Instead, Caddy's
`api-aaa` plugin handles authentication as a reverse proxy layer:

```
1. Client sends request to Caddy (port 1443 or 9999)
        |
2. Caddy api-aaa plugin intercepts /api/v1/* requests
        |
3. api-aaa POSTs to {EgperlApi}/api/v1/authentication/admin_authentication
   with {"username": "...", "password": "..."}
        |
4. Go handler (Authentication.AdminAuthentication):
   a. Reads username/password from JSON body
   b. Looks up password hash from the `password` table via store.GetPersonPassword()
   c. Strips optional {bcrypt} prefix from stored hash
   d. Validates with bcrypt.CompareHashAndPassword()
   e. Returns {"result": 1, "roles": ["ALL"]} on success
   f. Returns {"result": 0} on failure
        |
5. api-aaa sets X-Eguard-UserName and X-Eguard-Admin-Roles headers
        |
6. Caddy forwards the authenticated request to eg-api-server (port 22230)
        |
7. Go handlers trust the X-Eguard-* headers (no re-authentication)
```

**Critical contract**: The `result` field must be an integer (`1`), not a string
(`"1"`). The `api-aaa` plugin performs a strict type check and will reject string
values, breaking all API authentication.

### 14.6 Database Connection

The Go server uses GORM with the MySQL driver. Connection parameters:

| Parameter | Value | Source |
|-----------|-------|--------|
| DSN | MySQL connection string | `--db-uri` flag or `db.ReturnURIFromConfig()` |
| MaxIdleConns | 5 | Hardcoded in `store/db.go` |
| MaxOpenConns | 50 | Hardcoded in `store/db.go` |
| ConnMaxLifetime | 5 minutes | Hardcoded in `store/db.go` |
| GORM log level | Warn | Suppresses routine query logging |

The DSN is resolved in this order:

1. If `--db-uri` is provided on the command line, use it directly
2. Otherwise, call `db.ReturnURIFromConfig()` which reads the database URI from
   the eGuard configuration system (`egconfigdriver`)

No schema migrations are performed by the Go server. Both Perl and Go share the
same MySQL database and table schema.
...
```bash
# Enable and start the Go API server
sudo systemctl enable eguard-api-server
sudo systemctl start eguard-api-server

# Verify it is running
sudo systemctl status eguard-api-server
curl -s http://127.0.0.1:22230/healthz
# Expected: "ok"
```

Service unit details (`eguard-api-server.service`):

| Directive | Value | Purpose |
|-----------|-------|---------|
| Type | notify | Uses sd_notify for readiness signaling |
| ExecStart | `/usr/local/eg/sbin/eg-api-server --listen 127.0.0.1:22230` | Listens on loopback only (Caddy fronts it) |
| Conflicts | `eguard-perl-api.service` | Prevents both APIs from running simultaneously |
| User | eg | Runs as the unprivileged eGuard service account |
| Restart | on-failure | Automatic restart with 5s delay, max 3 bursts per 60s |
| NoNewPrivileges | true | Security hardening |
| ProtectSystem | strict | Read-only filesystem except explicit write paths |
| ReadWritePaths | `/usr/local/eg/var`, `/usr/local/eg/conf`, `/usr/local/eg/logs` | Allowed write directories |
...
The Go API server is exposed to clients through a Caddy drop-in config file:

```
# /usr/local/eg/conf/caddy-services/api.conf.d/endpoint-go-proxy.conf
reverse_proxy /api/v1/* 127.0.0.1:22230
```

This file is loaded by Caddy's API frontend service. When present, all
`/api/v1/*` requests are routed to the Go server instead of Perl.

### 14.8 Rollback Procedure

If the Go API server exhibits issues, rollback to Perl is immediate:

```bash
# 1. Remove the Caddy proxy config (routes revert to Perl backend)
sudo rm /usr/local/eg/conf/caddy-services/api.conf.d/endpoint-go-proxy.conf

# 2. Reload Caddy to pick up the config change
sudo systemctl restart eguard-api-frontend

# 3. Stop and disable the Go API server
sudo systemctl stop eguard-api-server
sudo systemctl disable eguard-api-server

# 4. Verify Perl API is serving requests
curl -s https://localhost:1443/api/v1/authentication/admin_authentication \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"test"}' -k
```

The Perl API (`eguard-perl-api.service`) resumes serving all `/api/v1/*` routes
automatically once the Go proxy config is removed.

### 14.9 Stubs (Not Yet Implemented)

The following features return placeholder responses and need real implementations:

| Feature | Handler | Notes |
|---------|---------|-------|
| Fingerbank | `handler/fingerbank.go` | Device fingerprinting integration |
| K8s Services | `handler/k8s_services.go` | Kubernetes cluster management |
| License | `handler/license.go` | Full license validation logic |
| Configurator | `handler/configurator.go` | Configuration proxy to backend services |
| Dynamic Reports | `handler/dynamic_reports.go` | Custom report generation backend |
| Queues | `handler/queues.go` | Redis-backed job queue status |
| Emails | `handler/emails.go` | Email preview and send functionality |

### 14.10 Known Quirks

**httprouter wildcard conflict.** The httprouter library does not allow a static
path segment and a wildcard parameter at the same level. For example,
`/endpoint-responses/:id` and `/endpoint-responses/quarantined` cannot coexist.
The workaround is a single wildcard route (`/endpoint-responses/:param`) with a
dispatch handler that inspects the parameter value and delegates accordingly.

**egconfigdriver panic.** If the binary is started without `--db-uri` and the
eGuard configuration environment is not available (e.g., `egconfigdriver` is not
running or `/usr/local/eg/conf` is missing), the `db.ReturnURIFromConfig()` call
will panic. Always provide `--db-uri` in development or testing environments
outside a full eGuard installation.

**Authentication result type.** The `{"result": 1}` response must use an integer
value, not a string `"1"`. The `api-aaa` Caddy plugin performs a strict type
check on this field. Returning a string will silently break all API
authentication with no error message in the Caddy logs.

**Graceful shutdown timeout.** The server uses a 10-second graceful shutdown
window (`context.WithTimeout`). Long-running requests (e.g., large report
exports) may be terminated during shutdown.

### 14.11 Port Assignments

| Port | Service | Binary | Status |
|------|---------|--------|--------|
| 22222 | egdhcp | egdhcp | Active |
| 22223 | egipset | egipset | Active |
| 22224 | egperl-api (Perl) | egperl-api | Legacy (replaced by Go) |
| 22225 | egpki | egpki | Active |
| 22230 | eg-api-server (Go) | eg-api-server | Active |
