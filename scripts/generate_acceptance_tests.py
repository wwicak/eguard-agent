...
def render_runtime_stubs(
    ...
    *,
    ...
    lines.append("")
    lines.append('const AC_STATUS_DOC: &str = include_str!("../AC_STATUS.md");')
    lines.append("")
    ...
    for item in criteria:
        ...
        description = rust_string(item.description)

        lines.append("#[test]")
        lines.append(f"fn {fn_name}() {{")
        lines.append(
            f'    assert_runtime_validation_backed_by_executable_suite("{item.ac_id}", "{description}");'
        )
        lines.append("}")
        lines.append("")
...
def main() -> None:
    ...
    write_generated(
        ...
        render_runtime_stubs(
            criteria,
            file_comment="// Runtime validation registry checks generated from acceptance criteria.",
    ...
    write_generated(
        ...
        render_runtime_stubs(
            criteria_tst,
            file_comment="// Focused AC-TST runtime validation registry checks.",
    ...
    write_generated(
        ...
        render_runtime_stubs(
            criteria_ver,
            file_comment="// Focused AC-VER runtime validation registry checks.",
            function_suffix="runtime_stub",
        ),
    )

    print(f"wrote {OUT_PRESENCE} with {len(criteria)} tests + integrity checks")
    print(f"wrote {OUT_RUNTIME_ALL} with {len(criteria)} runtime registry checks")
    print(f"wrote {OUT_RUNTIME_TST} with {len(criteria_tst)} runtime registry checks")
    print(f"wrote {OUT_RUNTIME_VER} with {len(criteria_ver)} runtime registry checks")
